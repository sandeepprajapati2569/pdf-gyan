...
import { BookOpen, ChevronDown, KeyRound, LogIn, LogOut, Menu, Moon, PanelsTopLeft, Settings2, Sparkles, Sun, TriangleAlert, X } from 'lucide-react'
...
export default function Navbar() {
  ...
  const navSurfaceClass =
    theme === 'dark'
      ? 'border-white/8 bg-[rgba(20,24,38,0.78)]'
      : 'border-white/65 bg-[rgba(255,255,255,0.68)]'

  const groupedSurfaceClass =
    theme === 'dark'
      ? 'border-white/8 bg-[rgba(30,35,50,0.76)]'
      : 'border-slate-200/80 bg-white/80'

  const utilityButtonClass =
    theme === 'dark'
      ? 'border-white/8 bg-[rgba(30,35,50,0.74)] text-slate-300 hover:text-white'
      : 'border-slate-200/80 bg-white/72 text-slate-500 hover:text-slate-700'

  const mobileMenuSurfaceClass =
    theme === 'dark'
      ? 'border-white/10 bg-[rgba(20,24,38,0.88)]'
      : 'border-white/65 bg-[rgba(255,255,255,0.76)]'

  const accountButtonClass = accountMenuOpen
    ? theme === 'dark'
      ? 'border-white/12 bg-[rgba(36,41,58,0.96)] shadow-[0_16px_36px_rgba(0,0,0,0.28)]'
      : 'border-slate-300/80 bg-white/90 shadow-[0_16px_36px_rgba(15,23,42,0.12)]'
    : theme === 'dark'
      ? 'border-white/8 bg-[rgba(30,35,50,0.74)]'
      : 'border-slate-200/80 bg-white/72'

  const accountMenuSurfaceClass =
    theme === 'dark'
      ? 'border-white/10 bg-[rgba(20,24,38,0.94)]'
      : 'border-white/70 bg-[rgba(255,255,255,0.88)]'

  const accountPanelClass =
    theme === 'dark'
      ? 'border-white/10 bg-[rgba(30,35,50,0.88)]'
      : 'border-slate-200/80 bg-white/90'

  const logoutButtonClass =
    theme === 'dark'
      ? 'border-white/10 bg-[rgba(30,35,50,0.82)] text-slate-200 hover:border-rose-400/30 hover:bg-rose-500/12 hover:text-rose-200'
      : 'border-slate-200/80 bg-white/90 text-slate-700 hover:border-rose-200 hover:bg-rose-50/90 hover:text-rose-700'

  const modalSurfaceClass =
    theme === 'dark'
      ? 'border-white/10 bg-[rgba(20,24,38,0.94)]'
      : 'border-white/70 bg-[rgba(255,255,255,0.92)]'
    ...
    <>
      ...
      <nav className="fixed inset-x-0 top-0 z-50 px-3 pt-3 sm:px-4 sm:pt-4">
        <div className="mx-auto max-w-7xl">
          <div className={`flex items-center justify-between gap-4 rounded-[30px] border px-4 py-3 shadow-[0_18px_55px_rgba(15,23,42,0.12)] backdrop-blur-2xl sm:px-5 ${navSurfaceClass}`}>
            <Link
              to={user ? '/dashboard' : '/'}
            ...
            <div className="hidden items-center gap-2 lg:flex">
              {user ? (
                <>
                  <div className={`flex items-center gap-1 rounded-full border p-1 ${groupedSurfaceClass}`}>
                  ...
                  <button
                    type="button"
                    onClick={toggleTheme}
                    className={`grid h-10 w-10 place-items-center rounded-2xl border transition hover:-translate-y-0.5 ${utilityButtonClass}`}
                  ...
                  >
                    <button
                      type="button"
                      className={`flex items-center gap-3 rounded-full border px-3 py-2 transition ${accountButtonClass}`}
                    >
                    ...
                    >
                      <div className={`rounded-[28px] border p-3 shadow-[0_22px_60px_rgba(15,23,42,0.16)] backdrop-blur-2xl ${accountMenuSurfaceClass}`}>
                        <div className={`rounded-[22px] border px-4 py-3 ${accountPanelClass}`}>
                        ...
                        <button
                          ...
                          className={`mt-3 flex w-full items-center justify-between rounded-[22px] border px-4 py-3 text-left text-sm font-semibold transition ${logoutButtonClass}`}
                        >
            ...
            <button
              ...
              className={`grid h-11 w-11 place-items-center rounded-2xl border transition hover:-translate-y-0.5 hover:border-teal-200 lg:hidden ${utilityButtonClass}`}
          ...
          {mobileOpen && (
            <div className={`mt-3 overflow-hidden rounded-[28px] border p-3 shadow-[0_18px_55px_rgba(15,23,42,0.12)] backdrop-blur-2xl lg:hidden ${mobileMenuSurfaceClass}`}>
      ...
      {logoutConfirmOpen && (
        <div className="fixed inset-0 z-[70] flex items-center justify-center bg-slate-950/28 px-4 backdrop-blur-sm" onClick={closeLogoutConfirm}>
          <div
            className={`w-full max-w-md rounded-[32px] border p-6 shadow-[0_28px_80px_rgba(15,23,42,0.24)] backdrop-blur-2xl sm:p-7 ${modalSurfaceClass}`}
